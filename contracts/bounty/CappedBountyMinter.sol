...
contract CappedBountyMinter is Ownable {
  ...
  constructor(address _token, uint256 _cap) public {
    require(_token != address(0), "Token shouldn't be the zero address.");
    require(_cap > 0, "Bounty cap should be greater than zero.");
  ...
  function multiSend(address[] addresses, uint256[] amounts) public onlyOwner {
    require(addresses.length > 0, "Addresses array shouldn't be empty.");
    require(amounts.length > 0, "Amounts array shouldn't be empty.");
    require(addresses.length == amounts.length, "Addresses and amounts arrays should have the same length.");
    ...
    for (uint i = 0; i < addresses.length; i++) {
      ...
      require(totalGivenBountyTokens <= cap, "Max bounty cap reached.");
